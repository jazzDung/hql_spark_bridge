...
class CurPySparkGenerator:
    def __init__(self, source_rules: Dict[str, Any], output_mode: str = "simple"):
        ...
        self.transformer = CurSparkTransformer()
        ...
        for model_folder in model_folders:
            ...
            for template_path in ddl_templates:
                ...
                try:
                    ...
                    output_filename = f"{pipeline_config['layer']}_{pipeline_config['target_table_name']}_{template_path.stem}.sql"
    ...
    def _generate_simple_dml(self, pipeline_config: Dict[str, Any], output_root: Path):
        ...
        for step in pipeline_config.get("source_processing_steps", []):
            ...
            target_table_name = f"{pipeline_config['target_table_name']}_{source_id}"
            ...
            if step_file.exists():
                ...
                for query_obj in jinja_render_model.transformed_queries:
                     if isinstance(query_obj, dict) and query_obj.get('type') == 'query':
                         source_processing_sqls.append(query_obj.get('content'))
                     elif isinstance(query_obj, str): # In case it's just a list of strings
                         source_processing_sqls.append(query_obj)
            ...
            for model_folder in model_folders:
                ...
                for template_path in dml_templates:
                    ...
                    try:
                        ...
                        output_filename = f"{pipeline_config['layer']}_{pipeline_config['target_table_name'].lower()}_{source_id.lower()}_{template_path.stem}.py"
