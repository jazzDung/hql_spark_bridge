...
class CurSqlDecomposer:
    ...
    def get_target_table(self, file_path: str) -> str:
        """
        Phân tích file .sql và trả về chính xác Tên Bảng đích cuối cùng.
        Xử lý mượt mà cả cú pháp PARTITION, tự động vứt bỏ Schema và Quotes.
        Format: lowercase (vd: 'dim_indicators').
        """
        ...
        try:
            statements = sqlglot.parse(sql_content, read="hive")
            for stmt in statements:
                ...
                # Bắt lệnh CREATE
                elif isinstance(stmt, exp.Create) and stmt.args.get("kind") == "TABLE":
                    table_node = stmt.this
                ...
                # Xử lý node để bóc tên bảng
                if table_node:
                    # Nếu lệnh có format (col1, col2), table_node sẽ bị bọc trong exp.Schema -> Lùi vào 1 cấp
                    if isinstance(table_node, exp.Schema):
                        table_node = table_node.this

                    # Lúc này table_node chắc chắn là exp.Table.
                    # Gọi .name để lấy tên bảng thuần túy (không schema, không partition, không quote)
                    if isinstance(table_node, exp.Table):
                        final_name = table_node.name.lower()
                        written_tables.append(final_name)

        except Exception as e:
            print(f"[Lỗi] Không thể parse file SQL: {e}")
            return None

        # Chiến thuật Duyệt Ngược (Bottom-Up)
        temp_keywords = ['temp_', 'tmp_', 'stg_', 'staging_']

        for tbl in reversed(written_tables):
            # Chốt bảng KHÔNG phải là bảng tạm!
            if not any(kw in tbl for kw in temp_keywords) and not tbl.endswith("_h"):
                return tbl

        # Fallback
        return written_tables[-1] if written_tables else None

    # ==========================================
    # TEST HÀM
    # ==========================================
    if __name__ == "__main__":
        # target = get_target_table_from_sql('DIM_CONTACT.sql')
        # print(target)
        pass
    ...
    def decompose(self, sql_file: Path) -> CurDecomposedScript:
        ...
        target_table = self.get_target_table(sql_file)
        ...
        for stmt in statements:
            ...
            mutated_table = self._extract_mutated_table_name(stmt)
            if mutated_table:
                if mutated_table.lower() == target_table:
    ...
    def _extract_mutated_table_name(self, stmt: exp.Expression) -> Optional[str]:
        # Đối với INSERT, CREATE, ALTER, DROP, target table luôn nằm ở node 'this'
        # Không dùng .find() vì nó sẽ bị lạc vào mệnh đề FROM/JOIN

        if isinstance(stmt, (exp.Create, exp.Drop, exp.Alter, exp.TruncateTable, exp.Insert)):
            target = stmt.this
            # Nếu là exp.Schema (INSERT INTO table(cols)), thì tên bảng nằm trong target.this
            if isinstance(target, exp.Schema):
                return target.this.name.upper()
            # Nếu là exp.Table (CREATE TABLE table...)
            if isinstance(target, exp.Table):
                return target.name.upper()

        elif isinstance(stmt, exp.Command) and "ANALYZE" in stmt.sql().upper():
            # ANALYZE là trường hợp đặc biệt, nó không có cấu trúc node chuẩn như Insert
            # Lúc này mới dùng .find() là hợp lý
            tbl = stmt.find(exp.Table)
            return tbl.name.upper() if tbl else None
