...
--    File Type		: DML
--    Purpose       : [Model 5A] Insert overwrite table's impacted partition(s) by delete and re-insert updated records based on records updated in COM_T. 
...
WHERE DATE_FORMAT(record_created_date, 'yyyyMMdd') = '${batch_date}' ;
...
WHERE cur.SOURCE_KEY = '<if partition applicable, else remove clause>' AND 
    NOT EXISTS -- get unchanged records based on date column and partition column(s)
...
SELECT 
    ...
    , raw_incremental_etl_dt  --when value is not null, it indicates data inserted incrementally (through Model 5A)
    , record_created_date 
    , record_updated_date 
