...
--    File Type		: DML
--    Purpose       : [Model 5A] Insert overwrite table by delete and re-insert updated records based on updated records in COM_T. 
...
WHERE cur.SOURCE_KEY = '<if partition applicable, else remove clause>' AND 
    NOT EXISTS --identify records not exists in COM_T delta
        ...
        WHERE WHERE delta.<datecolumn2> = cur.<datecolumn2>)
