...
--    File Type		: DML
--    Purpose		: [Model 3] Insert overwrite table by delete and re-insert updated records based on key column(s). 
...
WHERE NOT EXISTS 
    ...
    AND com_r.<keycolumn1> = com_t.<keycolumn1> AND com_r.<keycolumn2> = com_t.<keycolumn2> ) 
...
SELECT 
	...
	, r.<column3>	
	, r.<column4>	
    , r.<transformation logic 1> as <transformed_col1>
    , r.<transformation logic 2> as <transformed_col2>
    , 'A' as record_status 
...
LEFT JOIN ${com_schema}.t_smfkibb_tbl_account com_t 
