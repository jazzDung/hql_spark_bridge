...
--    File Type		: DML
--    Purpose       : [Model 6] Insert overwrite table by delete and re-insert updated records based on reference column(s). 
...
WHERE cur.SOURCE_KEY = '<if partition applicable, else remove clause>' AND 
    NOT EXISTS --identify records not exists in COM_T delta
        ...
        WHERE com_t.<keycolumn1> = cur.<keycolumn1> AND com_t.<keycolumn2> = cur.<keycolumn2>)


-- 2.3 Insert latest records from COM_T to temp table, identify COM_T delta based on record_updated_date 
