-- raw.mhbos_m_client_ext definition

CREATE TABLE `raw.mhbos_m_client_ext`(
  `client_no` string,
  `bank_accno` string,
  `bank_name` string,
  `bank_branch` string,
  ...
  `clr_acc_profit_pct` decimal(6,2),
  `clr_acc_loss_pct` decimal(6,2),
  ...
  `brk_sharing_intro_branch_level1_pct` decimal(5,2),
  `brk_sharing_intro_branch_tdr_level2_pct` decimal(5,2),
  `brk_sharing_exec_branch_tdr_level2_pct` decimal(5,2),
  `omnibus_no` string,
  ...
  `rollover_day` decimal(5,0),
  `last_rollover_date` timestamp,
  `tdr_code_ir` string,
  `tdr_code_tr` string,
  `percentage_ir` decimal(9,4),
  `percentage_tr` decimal(9,4),
  ...
  `installment_amt` decimal(14,2),
  `crs_tax` string,
  `leap_market` string,
  `vbip_flag` string,
  `vbip_group` string,
  `vbip_effective_date` timestamp,
  `vbip_threshold_amt` decimal(13,2),
  `vbip_rebate_percentage` decimal(5,2),
  `occupation_dup` string,
  `idss_flag` string,
  `idss_limit_percentage` decimal(5,2),
  `limit_multiplier` decimal(5,2),
  ...
  `app_ref_no` string,
  `fin_rate` decimal(5,2),
  `def_ori` decimal(5,2),
  `type_of_est` decimal(1,0),
  `probability_def` decimal(7,3),
  `loss_def` decimal(6,2),
...
LOCATION
  'maprfs:/hive_db/raw/mhbos_m_client_ext'
TBLPROPERTIES (
  'bucketing_version'='2',
  'external.table.purge'='true',
  'parquet.compression'='SNAPPY',
  'transient_lastDdlTime'='1776758698');