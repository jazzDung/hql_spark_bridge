...
spark.sql(f"""DROP TABLE IF EXISTS {params["tmp_schema"]}.temp_dim_address_final_consolidation_delta""")
spark.sql(f"""
    CREATE TABLE {params["tmp_schema"]}.temp_dim_address_final_consolidation_delta (
        owner_id VARCHAR(50)
...
spark.sql(f"""DROP TABLE IF EXISTS {params["tmp_schema"]}.temp_dim_address_final_consolidation_updated""")
spark.sql(f"""
    CREATE TABLE {params["tmp_schema"]}.temp_dim_address_final_consolidation_updated (
        owner_id VARCHAR(50)
        , address_owner_type VARCHAR(20)
        , address_type VARCHAR(10)
        , address_line_1 VARCHAR(100)
...
spark.sql(f"""
    INSERT INTO TABLE {params["tmp_schema"]}.temp_dim_address_final_consolidation_updated
    SELECT
        cur.owner_id,
    ...
    WHERE cur.source_key = 'FINAL_CONSOLIDATION' AND         NOT EXISTS (
            SELECT 1 FROM {params["tmp_schema"]}.temp_dim_address_final_consolidation_delta delta
...
spark.sql(f"""
    ...
    FROM {params["tmp_schema"]}.temp_dim_address_final_consolidation_delta delta
    LEFT JOIN {params["cur_schema"]}.dim_address_final_consolidation cur
        ON cur.source_key = 'FINAL_CONSOLIDATION' AND delta.OWNER_ID = cur.OWNER_ID AND delta.ADDRESS_OWNER_TYPE = cur.ADDRESS_OWNER_TYPE AND delta.ADDRESS_TYPE = cur.ADDRESS_TYPE""")

# ─── STEP 3: Insert outdated CUR records into History table ──────────────────
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.dim_address_final_consolidation_h PARTITION (source_key, hist_year)
    SELECT
        cur.owner_id,
        cur.address_owner_type,
        cur.address_type,
...
, 'FINAL_CONSOLIDATION' AS source_key        , DATE_FORMAT(current_timestamp(), 'yyyy') AS hist_year
    FROM {params["cur_schema"]}.dim_address_final_consolidation cur
    INNER JOIN {params["tmp_schema"]}.temp_dim_address_final_consolidation_delta delta
        ON delta.OWNER_ID = cur.OWNER_ID AND delta.ADDRESS_OWNER_TYPE = cur.ADDRESS_OWNER_TYPE AND delta.ADDRESS_TYPE = cur.ADDRESS_TYPE    WHERE cur.source_key = 'FINAL_CONSOLIDATION' AND         cur.hash_value <> delta.hash_value
""")

# ─── STEP 4: Overwrite CUR table ──────────────────────────────────────────────
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_address_final_consolidation PARTITION (source_key = 'FINAL_CONSOLIDATION')    SELECT
        owner_id,
        address_owner_type,
        address_type,
        address_line_1,
        ...
        '{batch_date}' AS etl_dt,
        current_timestamp() AS etl_timestamp
    FROM {params["tmp_schema"]}.temp_dim_address_final_consolidation_updated
""")

spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_address_final_consolidation PARTITION (source_key = 'FINAL_CONSOLIDATION') COMPUTE STATISTICS
""")
