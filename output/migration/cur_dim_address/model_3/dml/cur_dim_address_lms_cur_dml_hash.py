...
spark.sql(f"""DROP TABLE IF EXISTS {params["tmp_schema"]}.temp_dim_address_lms_delta""")
spark.sql(f"""
    CREATE TABLE {params["tmp_schema"]}.temp_dim_address_lms_delta (
        owner_id VARCHAR(50)
...
spark.sql(f"""DROP TABLE IF EXISTS {params["tmp_schema"]}.temp_dim_address_lms_updated""")
spark.sql(f"""
    CREATE TABLE {params["tmp_schema"]}.temp_dim_address_lms_updated (
        owner_id VARCHAR(50)
        , address_owner_type VARCHAR(20)
        , address_type VARCHAR(10)
        , address_line_1 VARCHAR(100)
...
spark.sql(f"""
    INSERT INTO TABLE {params["tmp_schema"]}.temp_dim_address_lms_updated
    SELECT
        cur.owner_id,
    ...
    WHERE cur.source_key = 'LMS' AND         NOT EXISTS (
            SELECT 1 FROM {params["tmp_schema"]}.temp_dim_address_lms_delta delta
...
spark.sql(f"""
    ...
    FROM {params["tmp_schema"]}.temp_dim_address_lms_delta delta
...
, 'LMS' AS source_key        , DATE_FORMAT(current_timestamp(), 'yyyy') AS hist_year
    FROM {params["cur_schema"]}.dim_address_lms cur
    INNER JOIN {params["tmp_schema"]}.temp_dim_address_lms_delta delta
        ON delta.OWNER_ID = cur.OWNER_ID AND delta.ADDRESS_OWNER_TYPE = cur.ADDRESS_OWNER_TYPE AND delta.ADDRESS_TYPE = cur.ADDRESS_TYPE    WHERE cur.source_key = 'LMS' AND         cur.hash_value <> delta.hash_value
""")
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_address_lms PARTITION (source_key = 'LMS')    SELECT
        ...
        '{batch_date}' AS etl_dt,
        current_timestamp() AS etl_timestamp
    FROM {params["tmp_schema"]}.temp_dim_address_lms_updated
""")
