...
, 'M21' AS source_key        , DATE_FORMAT(current_timestamp(), 'yyyy') AS hist_year
    FROM {params["cur_schema"]}.dim_address_m21 cur
    INNER JOIN {params["tmp_schema"]}.temp_dim_address_m21_delta delta
        ON delta.OWNER_ID = cur.OWNER_ID AND delta.ADDRESS_OWNER_TYPE = cur.ADDRESS_OWNER_TYPE AND delta.ADDRESS_TYPE = cur.ADDRESS_TYPE    WHERE cur.source_key = 'M21' AND         cur.hash_value <> delta.hash_value
""")
