...
spark.sql(f"""
    ...
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_dim_address_mhbos_delta
    ) impacted_partitions
...
spark.sql(f"""
    ...
    FROM {params["com_schema"]}.temp_dim_address_mhbos_delta delta
...
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.dim_address_mhbos_h PARTITION (source_key, <<partition_column>>, hist_year)
    SELECT
        cur.owner_id,
        cur.address_owner_type,
        cur.address_type,
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_address_mhbos PARTITION (source_key = 'MHBOS', <<partition_column>>)
    SELECT
        owner_id,
        address_owner_type,
        address_type,
...
spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_address_mhbos COMPUTE STATISTICS
""")
