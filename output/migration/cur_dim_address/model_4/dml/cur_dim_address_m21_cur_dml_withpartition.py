...
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_dim_address_m21_delta""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_dim_address_m21_delta (
        owner_id VARCHAR(50)
...
# ─── UPDATED TABLE SETUP ─────────────────────────────────────────────────────
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_dim_address_m21_updated""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_dim_address_m21_updated (
        owner_id VARCHAR(50)
        , address_owner_type VARCHAR(20)
        , address_type VARCHAR(10)
        , address_line_1 VARCHAR(100)
...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_dim_address_m21_updated
    SELECT
        cur.owner_id,
    ...
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_dim_address_m21_delta
    ) impacted_partitions
    ON cur.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
cur.source_key = 'M21' AND         NOT EXISTS (
            SELECT 1 FROM {params["com_schema"]}.temp_dim_address_m21_delta delta
...
spark.sql(f"""
    ...
    FROM {params["com_schema"]}.temp_dim_address_m21_delta delta
...
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.dim_address_m21_h PARTITION (source_key, <<partition_column>>, hist_year)
    SELECT
        cur.owner_id,
        cur.address_owner_type,
        cur.address_type,
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_address_m21 PARTITION (source_key = 'M21', <<partition_column>>)
    SELECT
        owner_id,
        address_owner_type,
        address_type,
        ...
        current_timestamp() AS etl_timestamp
        , <<partition_column>>
    FROM {params["com_schema"]}.temp_dim_address_m21_updated
""")

spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_address_m21 COMPUTE STATISTICS
""")
