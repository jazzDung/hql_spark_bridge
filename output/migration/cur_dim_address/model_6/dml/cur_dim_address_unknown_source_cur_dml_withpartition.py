...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_dim_address_unknown_source_delta
    SELECT
        com_t.owner_id,
    ...
    FROM {params["com_schema"]}.dim_address_unknown_source com_t
    INNER JOIN (
        SELECT DISTINCT OWNER_ID,ADDRESS_OWNER_TYPE,ADDRESS_TYPE        FROM {params["com_schema"]}.dim_address_unknown_source
...
spark.sql(f"""
    ...
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_dim_address_unknown_source_delta
    ) impacted_partitions
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_address_unknown_source PARTITION (source_key = 'UNKNOWN_SOURCE', <<partition_column>>)
    SELECT
        owner_id,
        address_owner_type,
        address_type,
...
spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_address_unknown_source COMPUTE STATISTICS
""")
