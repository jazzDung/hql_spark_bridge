...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_dim_address_lms_delta
    SELECT
        com_t.owner_id,
    ...
    FROM {params["com_schema"]}.dim_address_lms com_t
    INNER JOIN (
        SELECT DISTINCT OWNER_ID,ADDRESS_OWNER_TYPE,ADDRESS_TYPE        FROM {params["com_schema"]}.dim_address_lms
...
spark.sql(f"""
    ...
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_dim_address_lms_delta
    ) impacted_partitions
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_address_lms PARTITION (source_key = 'LMS', <<partition_column>>)
    SELECT
        owner_id,
        address_owner_type,
        address_type,
...
spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_address_lms COMPUTE STATISTICS
""")
