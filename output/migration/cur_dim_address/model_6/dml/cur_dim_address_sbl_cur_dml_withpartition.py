...
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_dim_address_sbl_delta""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_dim_address_sbl_delta (
        owner_id VARCHAR(50)
...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_dim_address_sbl_delta
    SELECT
        com_t.owner_id,
    ...
    FROM {params["com_schema"]}.dim_address_sbl com_t
    INNER JOIN (
        SELECT DISTINCT OWNER_ID,ADDRESS_OWNER_TYPE,ADDRESS_TYPE        FROM {params["com_schema"]}.dim_address_sbl
...
# ─── UPDATED TABLE SETUP ─────────────────────────────────────────────────────
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_dim_address_sbl_updated""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_dim_address_sbl_updated (
        owner_id VARCHAR(50)
        , address_owner_type VARCHAR(20)
        , address_type VARCHAR(10)
        , address_line_1 VARCHAR(100)
...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_dim_address_sbl_updated
    SELECT
        cur.owner_id,
    ...
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_dim_address_sbl_delta
    ) impacted_partitions
    ON cur.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
cur.source_key = 'SBL' AND         NOT EXISTS (
            SELECT 1 FROM {params["com_schema"]}.temp_dim_address_sbl_delta delta
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_address_sbl PARTITION (source_key = 'SBL', <<partition_column>>)
    SELECT
        owner_id,
        address_owner_type,
        address_type,
        ...
        current_timestamp() AS etl_timestamp
        , <<partition_column>>
    FROM {params["com_schema"]}.temp_dim_address_sbl_updated
""")

spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_address_sbl COMPUTE STATISTICS
""")
