...
spark.sql(f"""
/* Step 3: Get all records WITH exception */ /* Find exception records
...
spark.sql(f"""
/* Step 2: Generate New Cust ID */ /* Generate new cust_id
...
# ─── TEMP TABLE SETUP ────────────────────────────────────────────────────────
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_m_mhbos_m_client_updated""")
...
FROM {params["com_schema"]}.m_mhbos_m_client com_t
WHERE NOT EXISTS (
    SELECT 1 FROM {params["raw_schema"]}.mhbos_m_mhbos_m_client r
    WHERE r.etl_dt = '{batch_date}'
      AND r.cust_id = com_t.cust_id
)
""")
...
SELECT
    ...
    , 'A' AS dl_record_status
    , CASE
        WHEN com_t.cust_id IS NOT NULL THEN com_t.dl_record_created_date
        ELSE current_timestamp() END AS dl_record_created_date
    , current_timestamp() AS dl_record_updated_date
FROM {params["raw_schema"]}.mhbos_m_mhbos_m_client r
LEFT JOIN {params["com_schema"]}.m_mhbos_m_client com_t
    ON r.cust_id = com_t.cust_id
WHERE r.etl_dt = '{batch_date}'
""")
...
spark.sql(f"""ANALYZE TABLE {params["com_schema"]}.m_mhbos_m_client COMPUTE STATISTICS""")
