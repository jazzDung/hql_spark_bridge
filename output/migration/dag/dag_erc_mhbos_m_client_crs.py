...
# RAW
raw_mhbos_m_client_crs = BashOperator(
    task_id='raw_mhbos_m_client_crs',
    bash_command="""
        set -euo pipefail
        {{ var.value.spark_submit_config_low }} \
        {{ var.value.script_home_raw }}/raw_mhbos_m_client_crs.py
        """,
    env=env,
    dag=dag,
)
...
# COM_R
com_r_mhbos_m_client_crs = BashOperator(
    task_id='com_r_mhbos_m_client_crs',
    bash_command="""
        set -euo pipefail
        {{ var.value.spark_submit_config_low }} \
        {{ var.value.script_home_com }}/com_r_mhbos_m_client_crs.py
        """,
    dag=dag,
)

# COM_T
com_t_mhbos_m_client_crs = BashOperator(
    task_id='com_t_mhbos_m_client_crs',
    bash_command="""
        set -euo pipefail
        {{ var.value.spark_submit_config_low }} \
        {{ var.value.script_home_com }}/com_t_mhbos_m_client_crs.py
        """,
    dag=dag,
)
...
etl_run_start >> raw_mhbos_m_client_crs >> com_r_mhbos_m_client_crs >> com_t_mhbos_m_client_crs >> finish >> etl_run_end
