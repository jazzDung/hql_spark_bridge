...
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_dim_customer_final_consolidation_delta""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_dim_customer_final_consolidation_delta (
...
# ─── UPDATED TABLE SETUP ─────────────────────────────────────────────────────
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_dim_customer_final_consolidation_updated""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_dim_customer_final_consolidation_updated (
        customer_id VARCHAR(20)
        , customer_type VARCHAR(10)
        , customer_title VARCHAR(20)
        , customer_name VARCHAR(250)
...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_dim_customer_final_consolidation_updated
    SELECT
        cur.customer_id,
        cur.customer_type,
        cur.customer_title,
    ...
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_dim_customer_final_consolidation_delta
    ) impacted_partitions
    ON cur.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
cur.source_key = 'FINAL_CONSOLIDATION' AND         NOT EXISTS (
            SELECT 1 FROM {params["com_schema"]}.temp_dim_customer_final_consolidation_delta delta
...
spark.sql(f"""
    ...
    FROM {params["com_schema"]}.temp_dim_customer_final_consolidation_delta delta
...
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.dim_customer_final_consolidation_h PARTITION (source_key, <<partition_column>>, hist_year)
    SELECT
        cur.customer_id,
        cur.customer_type,
        cur.customer_title,
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_customer_final_consolidation PARTITION (source_key = 'FINAL_CONSOLIDATION', <<partition_column>>)
    SELECT
        customer_id,
        customer_type,
        customer_title,
        ...
        current_timestamp() AS etl_timestamp
        , <<partition_column>>
    FROM {params["com_schema"]}.temp_dim_customer_final_consolidation_updated
""")

spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_customer_final_consolidation COMPUTE STATISTICS
""")
