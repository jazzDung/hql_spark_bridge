...
spark.sql(f"""
    ...
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_dim_customer_guava_delta
    ) impacted_partitions
    ON cur.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
cur.source_key = 'GUAVA' AND         NOT EXISTS (
            SELECT 1 FROM {params["com_schema"]}.temp_dim_customer_guava_delta delta
...
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.dim_customer_guava_h PARTITION (source_key, <<partition_column>>, hist_year)
    SELECT
        cur.customer_id,
        cur.customer_type,
        cur.customer_title,
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_customer_guava PARTITION (source_key = 'GUAVA', <<partition_column>>)
    SELECT
        customer_id,
        customer_type,
        customer_title,
        ...
        current_timestamp() AS etl_timestamp
        , <<partition_column>>
    FROM {params["com_schema"]}.temp_dim_customer_guava_updated
""")
