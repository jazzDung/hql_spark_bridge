...
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.dim_customer_lmskibb2_h PARTITION (source_key, <<partition_column>>, hist_year)
    SELECT
        cur.customer_id,
        cur.customer_type,
        cur.customer_title,
        ...
        current_timestamp() AS etl_timestamp
, 'LMSKIBB2' AS source_key        , cur.<<partition_column>>
    ...
    ON cur.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
cur.source_key = 'LMSKIBB2' AND         EXISTS (
            SELECT 1 FROM {params["com_schema"]}.temp_dim_customer_lmskibb2_delta delta
            WHERE delta.CUSTOMER_ID = cur.CUSTOMER_ID        )
""")
