...
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_dim_customer_toms_delta""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_dim_customer_toms_delta (
...
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_dim_customer_toms_updated""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_dim_customer_toms_updated (
        snapshot_date DATE,
        customer_id VARCHAR(20),
        customer_type VARCHAR(10),
        customer_title VARCHAR(20),
...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_dim_customer_toms_updated
    SELECT
        snapshot_date,
        customer_id,
        customer_type,
    ...
    FROM {params["cur_schema"]}.dim_customer_toms
...
spark.sql(f"""
    ...
    FROM {params["com_schema"]}.temp_dim_customer_toms_delta delta
""")
...
""")
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_customer_toms
...
,'TOMS' AS source_key    FROM {params["com_schema"]}.temp_dim_customer_toms_updated
""")

spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_customer_toms COMPUTE STATISTICS
""")
