...
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_dim_customer_guava_delta""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_dim_customer_guava_delta (
...
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_dim_customer_guava_updated""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_dim_customer_guava_updated (
        snapshot_date DATE,
        customer_id VARCHAR(20),
        customer_type VARCHAR(10),
        customer_title VARCHAR(20),
...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_dim_customer_guava_updated
    SELECT
        snapshot_date,
        customer_id,
        customer_type,
    ...
    FROM {params["cur_schema"]}.dim_customer_guava
...
spark.sql(f"""
    ...
    FROM {params["com_schema"]}.temp_dim_customer_guava_delta delta
""")
...
""")
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_customer_guava
...
,'GUAVA' AS source_key    FROM {params["com_schema"]}.temp_dim_customer_guava_updated
""")

spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_customer_guava COMPUTE STATISTICS
""")
