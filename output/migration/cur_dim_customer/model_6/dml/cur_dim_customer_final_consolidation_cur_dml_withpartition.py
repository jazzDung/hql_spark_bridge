...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_dim_customer_final_consolidation_delta
    SELECT
        com_t.customer_id,
        com_t.customer_type,
    ...
    FROM {params["com_schema"]}.dim_customer_final_consolidation com_t
    INNER JOIN (
        SELECT DISTINCT CUSTOMER_ID        FROM {params["com_schema"]}.dim_customer_final_consolidation
...
cur.source_key = 'FINAL_CONSOLIDATION' AND         NOT EXISTS (
            ...
            WHERE delta.CUSTOMER_ID = cur.CUSTOMER_ID AND delta.<<partition_column>> = cur.<<partition_column>>        )
""")
