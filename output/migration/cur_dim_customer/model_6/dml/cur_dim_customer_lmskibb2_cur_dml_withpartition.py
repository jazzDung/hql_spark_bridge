...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_dim_customer_lmskibb2_delta
    SELECT
        com_t.customer_id,
        com_t.customer_type,
    ...
    FROM {params["com_schema"]}.dim_customer_lmskibb2 com_t
    INNER JOIN (
        SELECT DISTINCT CUSTOMER_ID        FROM {params["com_schema"]}.dim_customer_lmskibb2
...
spark.sql(f"""
    ...
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_dim_customer_lmskibb2_delta
    ) impacted_partitions
    ON cur.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
cur.source_key = 'LMSKIBB2' AND         NOT EXISTS (
            SELECT 1 FROM {params["com_schema"]}.temp_dim_customer_lmskibb2_delta delta
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_customer_lmskibb2 PARTITION (source_key = 'LMSKIBB2', <<partition_column>>)
    SELECT
        customer_id,
        customer_type,
        customer_title,
        ...
        current_timestamp() AS etl_timestamp
        , <<partition_column>>
    FROM {params["com_schema"]}.temp_dim_customer_lmskibb2_updated
""")
