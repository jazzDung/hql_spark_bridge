...
spark.sql(f"""
    ...
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_dim_customer_unknown_source_delta
    ) impacted_partitions
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_customer_unknown_source PARTITION (source_key = 'UNKNOWN_SOURCE', <<partition_column>>)
    SELECT
        customer_id,
        customer_type,
        customer_title,
...
spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_customer_unknown_source COMPUTE STATISTICS
""")
