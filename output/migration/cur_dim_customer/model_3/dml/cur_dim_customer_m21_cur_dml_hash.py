...
spark.sql(f"""DROP TABLE IF EXISTS {params["tmp_schema"]}.temp_dim_customer_m21_delta""")
spark.sql(f"""
    CREATE TABLE {params["tmp_schema"]}.temp_dim_customer_m21_delta (
...
spark.sql(f"""DROP TABLE IF EXISTS {params["tmp_schema"]}.temp_dim_customer_m21_updated""")
spark.sql(f"""
    CREATE TABLE {params["tmp_schema"]}.temp_dim_customer_m21_updated (
        customer_id VARCHAR(20)
        , customer_type VARCHAR(10)
        , customer_title VARCHAR(20)
        , customer_name VARCHAR(250)
...
spark.sql(f"""
    INSERT INTO TABLE {params["tmp_schema"]}.temp_dim_customer_m21_updated
    SELECT
        cur.customer_id,
        cur.customer_type,
        cur.customer_title,
    ...
    FROM {params["cur_schema"]}.dim_customer_m21 cur
    WHERE cur.source_key = 'M21' AND         NOT EXISTS (
            SELECT 1 FROM {params["tmp_schema"]}.temp_dim_customer_m21_delta delta
...
spark.sql(f"""
    ...
    FROM {params["tmp_schema"]}.temp_dim_customer_m21_delta delta
    LEFT JOIN {params["cur_schema"]}.dim_customer_m21 cur
        ON cur.source_key = 'M21' AND delta.CUSTOMER_ID = cur.CUSTOMER_ID""")

# ─── STEP 3: Insert outdated CUR records into History table ──────────────────
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.dim_customer_m21_h PARTITION (source_key, hist_year)
    SELECT
        cur.customer_id,
        cur.customer_type,
        cur.customer_title,
...
, 'M21' AS source_key        , DATE_FORMAT(current_timestamp(), 'yyyy') AS hist_year
    FROM {params["cur_schema"]}.dim_customer_m21 cur
    INNER JOIN {params["tmp_schema"]}.temp_dim_customer_m21_delta delta
        ON delta.CUSTOMER_ID = cur.CUSTOMER_ID    WHERE cur.source_key = 'M21' AND         cur.hash_value <> delta.hash_value
""")

# ─── STEP 4: Overwrite CUR table ──────────────────────────────────────────────
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_customer_m21 PARTITION (source_key = 'M21')    SELECT
        customer_id,
        customer_type,
        customer_title,
        customer_name,
        ...
        '{batch_date}' AS etl_dt,
        current_timestamp() AS etl_timestamp
    FROM {params["tmp_schema"]}.temp_dim_customer_m21_updated
""")

spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_customer_m21 PARTITION (source_key = 'M21') COMPUTE STATISTICS
""")
