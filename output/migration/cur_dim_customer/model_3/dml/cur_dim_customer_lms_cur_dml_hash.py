...
spark.sql(f"""DROP TABLE IF EXISTS {params["tmp_schema"]}.temp_dim_customer_lms_delta""")
spark.sql(f"""
    CREATE TABLE {params["tmp_schema"]}.temp_dim_customer_lms_delta (
...
spark.sql(f"""DROP TABLE IF EXISTS {params["tmp_schema"]}.temp_dim_customer_lms_updated""")
spark.sql(f"""
    CREATE TABLE {params["tmp_schema"]}.temp_dim_customer_lms_updated (
        customer_id VARCHAR(20)
        , customer_type VARCHAR(10)
        , customer_title VARCHAR(20)
        , customer_name VARCHAR(250)
...
spark.sql(f"""
    INSERT INTO TABLE {params["tmp_schema"]}.temp_dim_customer_lms_updated
    SELECT
        cur.customer_id,
        cur.customer_type,
        cur.customer_title,
    ...
    FROM {params["cur_schema"]}.dim_customer_lms cur
    WHERE cur.source_key = 'LMS' AND         NOT EXISTS (
            SELECT 1 FROM {params["tmp_schema"]}.temp_dim_customer_lms_delta delta
...
spark.sql(f"""
    ...
    FROM {params["tmp_schema"]}.temp_dim_customer_lms_delta delta
    LEFT JOIN {params["cur_schema"]}.dim_customer_lms cur
        ON cur.source_key = 'LMS' AND delta.CUSTOMER_ID = cur.CUSTOMER_ID""")

# ─── STEP 3: Insert outdated CUR records into History table ──────────────────
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.dim_customer_lms_h PARTITION (source_key, hist_year)
    SELECT
        cur.customer_id,
        cur.customer_type,
        cur.customer_title,
...
, 'LMS' AS source_key        , DATE_FORMAT(current_timestamp(), 'yyyy') AS hist_year
    FROM {params["cur_schema"]}.dim_customer_lms cur
    INNER JOIN {params["tmp_schema"]}.temp_dim_customer_lms_delta delta
        ON delta.CUSTOMER_ID = cur.CUSTOMER_ID    WHERE cur.source_key = 'LMS' AND         cur.hash_value <> delta.hash_value
""")

# ─── STEP 4: Overwrite CUR table ──────────────────────────────────────────────
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_customer_lms PARTITION (source_key = 'LMS')    SELECT
        customer_id,
        customer_type,
        customer_title,
        customer_name,
        ...
        '{batch_date}' AS etl_dt,
        current_timestamp() AS etl_timestamp
    FROM {params["tmp_schema"]}.temp_dim_customer_lms_updated
""")

spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_customer_lms PARTITION (source_key = 'LMS') COMPUTE STATISTICS
""")
