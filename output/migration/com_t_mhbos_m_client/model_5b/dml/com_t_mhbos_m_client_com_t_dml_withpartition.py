...
# ─── STEP 1: Keep unchanged records based on date column from IMPACTED PARTITIONS ───
spark.sql(f"""
    ...
    ON com_t.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
        ...
        AND NOT EXISTS (
            ...
            WHERE r.<<key_date_column>> = com_t.<<key_date_column>>
 AND r.<<partition_column>> = com_t.<<partition_column>>        )
""")
