...
# main_processing_sqls is expected to read and transform data from RAW for the current batch_date
...
# ─── STEP 1: Keep existing records from previous batches ─────────────────────
# This step ensures we don't re-process an etl_dt that's already in com_t
spark.sql(f"""
    ...
    WHERE raw_incremental_etl_dt != '{batch_date}'
""")
...
# ─── STEP 3: Overwrite target table with combined data ───────────────────────
spark.sql(f"""
