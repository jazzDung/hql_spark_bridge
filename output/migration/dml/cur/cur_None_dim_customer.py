...
##  File Type   : DML
##  Model       : 3a
##  Generated   : 2026-05-14 11:42:18
##  Source      : cur_dim_customer (migrated from Datalake Old)
...
INSERT INTO {params["cur_schema"]}.TEMP_DIM_CUSTOMER_MHBOS (
    ...
    LEFT JOIN (
      SELECT DISTINCT
        CLIENT_NO
      FROM {params["com_schema"]}.R_MHBOS_M_CLIENT_CRS
      WHERE
        ETL_DT = '{batch_date}'
    ) AS T3 /* None */
    ...
    LEFT JOIN (
      ...
      LEFT JOIN (
        SELECT DISTINCT
          CIFID,
          ALIASVALUE
        FROM {params["com_schema"]}.R_K2_CIF_ALIAS
        WHERE
          ALIASTYPE = 'BUSREGNO'
        ...
        ON B.CIFID = D.CIFID
      LEFT JOIN {params["com_schema"]}.R_K2_CIF_EXT AS E
        ON B.CIFID = E.CIFID
        AND E.RECSTATUS = 'AA'
...
spark.sql(f"""
/* ==============[Group.2]============== */
    DROP TABLE IF EXISTS {params["cur_schema"]}.TEMP_DIM_CUSTOMER_GUAVA_COMPANY
""")

spark.sql(f"""
CREATE TABLE {params["cur_schema"]}.TEMP_DIM_CUSTOMER_GUAVA_COMPANY (
      CUSTOMER_ID VARCHAR(20), /* None */
      CUSTOMER_TYPE VARCHAR(10), /* None */
      CUSTOMER_TITLE VARCHAR(20), /* None */
      CUSTOMER_NAME VARCHAR(250), /* None */
...
INSERT INTO {params["cur_schema"]}.TEMP_DIM_CUSTOMER_M21 (
    ...
    LEFT JOIN (
      ...
      WHERE
        DECLARATION_TYPE = 'PDPA' AND SOURCE_NAME = 'M21' AND ETL_DT = '{batch_date}'
    ) AS T2 /* None */
    ...
    LEFT JOIN (
      ...
      WHERE
        ETL_DT = '{batch_date}' AND SOURCE_NAME = 'M21'
    ) AS T4 /* None */
    ...
    LEFT JOIN {params["com_schema"]}.T_M21_STATEMENTRECIPIENT AS T6
      ON T6.CODE = T1.CODE AND T6.ETL_DT = '{batch_date}'
    WHERE
      T1.ETL_DT = '{batch_date}' AND NOT T1.CLEAN_RULE_FLAG LIKE '%1%'
""")
...
INSERT INTO {params["cur_schema"]}.TEMP_DIM_CUSTOMER_M21_A (
    ...
    LEFT JOIN (
      ...
      FROM {params["cur_schema"]}.REF_COUNTRY
      WHERE
        TRIM(COALESCE(COUNTRY_CODE_3_DIGITS, '')) <> '' AND ETL_DT = '{batch_date}'
    ) AS T2 /* None */
...
spark.sql(f"""
/* ==============[Group.9]============== */
    DROP TABLE IF EXISTS {params["cur_schema"]}.TEMP_DIM_CUSTOMER_SBL
""")

spark.sql(f"""
CREATE TABLE {params["cur_schema"]}.TEMP_DIM_CUSTOMER_SBL (
      CUSTOMER_ID VARCHAR(20), /* None */
      CUSTOMER_TYPE VARCHAR(10), /* None */
      CUSTOMER_TITLE VARCHAR(20), /* None */
      CUSTOMER_NAME VARCHAR(250), /* None */
...
spark.sql(f"""
/* ==============[Group.12]============== */
    DROP TABLE IF EXISTS {params["cur_schema"]}.TEMP_DIM_CUSTOMER_MERGE
""")

spark.sql(f"""
CREATE TABLE {params["cur_schema"]}.TEMP_DIM_CUSTOMER_MERGE (
      CUSTOMER_ID VARCHAR(20), /* None */
      CUSTOMER_TYPE VARCHAR(10), /* None */
      CUSTOMER_TITLE VARCHAR(20), /* None */
      CUSTOMER_NAME VARCHAR(250), /* None */
...
spark.sql(f"""
/* ==============[Group.11]============== */
    INSERT INTO {params["cur_schema"]}.DIM_CUSTOMER PARTITION(etl_dt = '{batch_date}') (
      CUSTOMER_ID, /* None */
      CUSTOMER_TYPE, /* None */
      CUSTOMER_TITLE, /* None */
      CUSTOMER_NAME, /* None */
...
spark.sql(f"""
DROP TABLE IF EXISTS {params["cur_schema"]}.TEMP_DIM_CUSTOMER_M21A
""")

spark.sql(f"""
DROP TABLE IF EXISTS {params["cur_schema"]}.TEMP_DIM_CUSTOMER_M21O
""")

spark.sql(f"""
DROP TABLE IF EXISTS {params["cur_schema"]}.TEMP_DIM_CUSTOMER_TOMS_ECORPORATE
""")
...
SELECT
    ...
    , r.customer_title
    , r.customer_name
    , r.customer_primary_identification_no_type
    , r.customer_primary_identification_no
    , r.customer_primary_identification_no_expiry_date
    , r.customer_secondary_identification_no_type
    , r.customer_secondary_identification_no
    , r.customer_secondary_identification_no_expiry_date
    , r.customer_nationality
    , r.customer_country_of_residence
    , r.customer_country_of_birth
    , r.customer_date_of_birth
    , r.customer_bumiputra_status
    , r.customer_race
    , r.customer_gender
    , r.customer_marital_status
    , r.customer_company_contact_person
    , r.customer_company_ownership
    , r.customer_company_country_of_registration
    , r.customer_company_type_of_business
    , r.customer_company_date_of_incorporation
    , r.customer_company_website
    , r.customer_company_type_of_organization
    , r.pdpa_flag
    , r.connected_party_flag
    , r.politically_exposed_person_flag
    , r.cross_selling_consent_flag
    , r.dcf_flag
    , r.multi_trading_account_flag
    , r.fatca_flag
    , r.crs_flag
    , r.customer_company_personnel_designation
    , r.customer_residency_status
    , r.auto_einvoice_indicator
    , r.sst_registration_no
    , r.customer_company_country_of_business
    , r.vulnerable_flag
    , 'A' AS dl_record_status
    , CASE
