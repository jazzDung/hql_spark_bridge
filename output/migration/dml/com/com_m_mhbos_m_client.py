...
WITH latest_primary_identification_no /* latest mapping based ON primary_identification_no */ AS (
  ...
  LEFT JOIN {params["com_schema"]}.t_mhbos_m_client AS t
    ...
    AND t.dl_record_updated_date = TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP('{batch_date}', 'yyyyMMdd'))) 
  WHERE
...
WHERE NOT EXISTS (
    ...
    WHERE r.dl_record_updated_date = TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP('{batch_date}', 'yyyyMMdd'))) 
