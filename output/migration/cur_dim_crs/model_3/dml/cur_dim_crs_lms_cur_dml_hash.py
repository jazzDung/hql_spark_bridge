...
spark.sql(f"""DROP TABLE IF EXISTS {params["tmp_schema"]}.temp_dim_crs_lms_delta""")
spark.sql(f"""
    CREATE TABLE {params["tmp_schema"]}.temp_dim_crs_lms_delta (
...
spark.sql(f"""DROP TABLE IF EXISTS {params["tmp_schema"]}.temp_dim_crs_lms_updated""")
spark.sql(f"""
    CREATE TABLE {params["tmp_schema"]}.temp_dim_crs_lms_updated (
        customer_id VARCHAR(20)
        , crs_entity_type VARCHAR(2)
        , name_of_controlling_person_1 VARCHAR(100)
        , name_of_controlling_person_2 VARCHAR(100)
...
spark.sql(f"""
    INSERT INTO TABLE {params["tmp_schema"]}.temp_dim_crs_lms_updated
    SELECT
        cur.customer_id,
        cur.crs_entity_type,
    ...
    FROM {params["cur_schema"]}.dim_crs_lms cur
    WHERE cur.source_key = 'LMS' AND         NOT EXISTS (
            SELECT 1 FROM {params["tmp_schema"]}.temp_dim_crs_lms_delta delta
...
spark.sql(f"""
    ...
    FROM {params["tmp_schema"]}.temp_dim_crs_lms_delta delta
    LEFT JOIN {params["cur_schema"]}.dim_crs_lms cur
        ON cur.source_key = 'LMS' AND delta.SOURCE_NAME = cur.SOURCE_NAME AND delta.CUSTOMER_ID = cur.CUSTOMER_ID AND delta.CRS_COUNTRY = cur.CRS_COUNTRY AND delta.TAXPAYER_IDENTIFICATION_NO_1 = cur.TAXPAYER_IDENTIFICATION_NO_1""")

# ─── STEP 3: Insert outdated CUR records into History table ──────────────────
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.dim_crs_lms_h PARTITION (source_key, hist_year)
    SELECT
        cur.customer_id,
        cur.crs_entity_type,
        cur.name_of_controlling_person_1,
...
, 'LMS' AS source_key        , DATE_FORMAT(current_timestamp(), 'yyyy') AS hist_year
    FROM {params["cur_schema"]}.dim_crs_lms cur
    INNER JOIN {params["tmp_schema"]}.temp_dim_crs_lms_delta delta
        ON delta.SOURCE_NAME = cur.SOURCE_NAME AND delta.CUSTOMER_ID = cur.CUSTOMER_ID AND delta.CRS_COUNTRY = cur.CRS_COUNTRY AND delta.TAXPAYER_IDENTIFICATION_NO_1 = cur.TAXPAYER_IDENTIFICATION_NO_1    WHERE cur.source_key = 'LMS' AND         cur.hash_value <> delta.hash_value
""")

# ─── STEP 4: Overwrite CUR table ──────────────────────────────────────────────
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_crs_lms PARTITION (source_key = 'LMS')    SELECT
        customer_id,
        crs_entity_type,
        ...
        '{batch_date}' AS etl_dt,
        current_timestamp() AS etl_timestamp
    FROM {params["tmp_schema"]}.temp_dim_crs_lms_updated
""")

spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_crs_lms PARTITION (source_key = 'LMS') COMPUTE STATISTICS
""")
