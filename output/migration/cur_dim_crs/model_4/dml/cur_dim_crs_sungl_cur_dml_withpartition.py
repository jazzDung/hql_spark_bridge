...
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_dim_crs_sungl_delta""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_dim_crs_sungl_delta (
...
# ─── UPDATED TABLE SETUP ─────────────────────────────────────────────────────
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_dim_crs_sungl_updated""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_dim_crs_sungl_updated (
        customer_id VARCHAR(20)
        , crs_entity_type VARCHAR(2)
        , name_of_controlling_person_1 VARCHAR(100)
        , name_of_controlling_person_2 VARCHAR(100)
...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_dim_crs_sungl_updated
    SELECT
        cur.customer_id,
        cur.crs_entity_type,
    ...
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_dim_crs_sungl_delta
    ) impacted_partitions
    ON cur.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
cur.source_key = 'SUNGL' AND         NOT EXISTS (
            SELECT 1 FROM {params["com_schema"]}.temp_dim_crs_sungl_delta delta
...
spark.sql(f"""
    ...
    FROM {params["com_schema"]}.temp_dim_crs_sungl_delta delta
...
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.dim_crs_sungl_h PARTITION (source_key, <<partition_column>>, hist_year)
    SELECT
        cur.customer_id,
        cur.crs_entity_type,
        cur.name_of_controlling_person_1,
        ...
        current_timestamp() AS etl_timestamp
, 'SUNGL' AS source_key        , cur.<<partition_column>>
    ...
    ON cur.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
cur.source_key = 'SUNGL' AND         EXISTS (
            SELECT 1 FROM {params["com_schema"]}.temp_dim_crs_sungl_delta delta
            WHERE delta.SOURCE_NAME = cur.SOURCE_NAME AND delta.CUSTOMER_ID = cur.CUSTOMER_ID AND delta.CRS_COUNTRY = cur.CRS_COUNTRY AND delta.TAXPAYER_IDENTIFICATION_NO_1 = cur.TAXPAYER_IDENTIFICATION_NO_1        )
""")
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_crs_sungl PARTITION (source_key = 'SUNGL', <<partition_column>>)
    SELECT
        customer_id,
        crs_entity_type,
        ...
        current_timestamp() AS etl_timestamp
        , <<partition_column>>
    FROM {params["com_schema"]}.temp_dim_crs_sungl_updated
""")

spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_crs_sungl COMPUTE STATISTICS
""")
