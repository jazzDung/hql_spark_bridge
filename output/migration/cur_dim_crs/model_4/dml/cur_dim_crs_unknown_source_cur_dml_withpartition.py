...
spark.sql(f"""
    ...
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_dim_crs_unknown_source_delta
    ) impacted_partitions
...
spark.sql(f"""
    ...
    FROM {params["com_schema"]}.temp_dim_crs_unknown_source_delta delta
...
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.dim_crs_unknown_source_h PARTITION (source_key, <<partition_column>>, hist_year)
    SELECT
        cur.customer_id,
        cur.crs_entity_type,
        cur.name_of_controlling_person_1,
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_crs_unknown_source PARTITION (source_key = 'UNKNOWN_SOURCE', <<partition_column>>)
    SELECT
        customer_id,
        crs_entity_type,
...
spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_crs_unknown_source COMPUTE STATISTICS
""")
