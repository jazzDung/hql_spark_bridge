...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_dim_crs_smf_delta
    SELECT
        com_t.customer_id,
    ...
    FROM {params["com_schema"]}.dim_crs_smf com_t
    INNER JOIN (
        SELECT DISTINCT SOURCE_NAME,CUSTOMER_ID,CRS_COUNTRY,TAXPAYER_IDENTIFICATION_NO_1        FROM {params["com_schema"]}.dim_crs_smf
...
spark.sql(f"""
    ...
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_dim_crs_smf_delta
    ) impacted_partitions
    ON cur.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
cur.source_key = 'SMF' AND         NOT EXISTS (
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.dim_crs_smf PARTITION (source_key = 'SMF', <<partition_column>>)
    SELECT
        customer_id,
        crs_entity_type,
...
spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.dim_crs_smf COMPUTE STATISTICS
""")
