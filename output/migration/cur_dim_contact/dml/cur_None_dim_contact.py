...
spark.sql(f"""
/* ==============[Group.49]============== */
    DROP TABLE IF EXISTS {params["cur_schema"]}.TEMP_DIM_CUSTOMER_CONTACT
""")

spark.sql(f"""
CREATE TABLE {params["cur_schema"]}.TEMP_DIM_CUSTOMER_CONTACT (
      OWNER_ID VARCHAR(50), /* None */
      CONTACT_OWNER_TYPE VARCHAR(20), /* None */
      CONTACT_TYPE VARCHAR(15), /* None */
      CONTACT_VALUE VARCHAR(150), /* None */
...
spark.sql(f"""
/* ==============[Group.54]============== */
    INSERT INTO {params["cur_schema"]}.DIM_CONTACT PARTITION(etl_dt = '{batch_date}') (
      OWNER_ID, /* None */
      CONTACT_OWNER_TYPE, /* None */
      CONTACT_TYPE, /* None */
      CONTACT_VALUE, /* None */
...
# ─── TEMP TABLE SETUP ────────────────────────────────────────────────────────
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_None_dim_contact_updated""")

spark.sql(f"""
CREATE TABLE {params["com_schema"]}.temp_None_dim_contact_updated (
    owner_id VARCHAR(50)
...
spark.sql(f"""
INSERT INTO TABLE {params["com_schema"]}.temp_None_dim_contact_updated
SELECT
    owner_id
    , contact_owner_type
    , contact_type
...
FROM {params["com_schema"]}.None_dim_contact com_t
WHERE NOT EXISTS (
    SELECT 1 FROM {params["raw_schema"]}.dim_None_dim_contact r
    WHERE r.etl_dt = '{batch_date}'
      AND r.owner_id = com_t.owner_id
)
""")
...
SELECT
    ...
    , 'A' AS dl_record_status
    , CASE
        WHEN com_t.owner_id IS NOT NULL THEN com_t.dl_record_created_date
        ELSE current_timestamp() END AS dl_record_created_date
    , current_timestamp() AS dl_record_updated_date
FROM {params["raw_schema"]}.dim_None_dim_contact r
LEFT JOIN {params["com_schema"]}.None_dim_contact com_t
    ON r.owner_id = com_t.owner_id
WHERE r.etl_dt = '{batch_date}'
""")

# ─── STEP 3: Overwrite target table ──────────────────────────────────────────
spark.sql(f"""
INSERT OVERWRITE TABLE {params["com_schema"]}.None_dim_contact
SELECT
    owner_id
    , contact_owner_type
    , contact_type
    ...
    , current_timestamp()  AS etl_timestamp
FROM {params["com_schema"]}.temp_None_dim_contact_updated
""")

spark.sql(f"""ANALYZE TABLE {params["com_schema"]}.None_dim_contact COMPUTE STATISTICS""")
