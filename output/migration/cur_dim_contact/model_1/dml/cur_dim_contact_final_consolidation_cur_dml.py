...
spark.sql(f"""
    ...
    WHERE cur.['source_key'] = 'FINAL_CONSOLIDATION' AND NOT EXISTS (
        SELECT 1 FROM {params["com_schema"]}.temp_DIM_CONTACT_FINAL_CONSOLIDATION_delta delta
        WHERE cur.OWNER_ID = delta.OWNER_ID AND cur.CONTACT_OWNER_TYPE = delta.CONTACT_OWNER_TYPE AND cur.CONTACT_TYPE = delta.CONTACT_TYPE    )
""")
