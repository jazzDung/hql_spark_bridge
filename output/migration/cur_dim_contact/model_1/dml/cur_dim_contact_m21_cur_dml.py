...
spark.sql(f"""
    ...
    FROM {params["com_schema"]}.temp_DIM_CONTACT_M21_delta delta
    LEFT JOIN {params["cur_schema"]}.DIM_CONTACT_M21 cur
        ON cur.['source_key'] = 'M21' AND delta.OWNER_ID = cur.OWNER_ID AND delta.CONTACT_OWNER_TYPE = cur.CONTACT_OWNER_TYPE AND delta.CONTACT_TYPE = cur.CONTACT_TYPE""")
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.DIM_CONTACT_M21 PARTITION (['source_key'] = 'M21')    SELECT
        owner_id
        , contact_owner_type
        , contact_type
        , contact_value
...
spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.DIM_CONTACT_M21 PARTITION (['source_key'] = 'M21') COMPUTE STATISTICS
""")
