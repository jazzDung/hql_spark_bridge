...
spark.sql(f"""
    ...
    FROM {params["cur_schema"]}.DIM_CONTACT_FINAL_CONSOLIDATION
    WHERE year_month = '{snapshot_year_month}'
      AND snapshot_date != to_date('{snapshot_date_str}', 'yyyyMMdd')
AND ['source_key'] = 'FINAL_CONSOLIDATION'""")
...
""")
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.DIM_CONTACT_FINAL_CONSOLIDATION
...
,'FINAL_CONSOLIDATION' AS ['source_key']    FROM {params["com_schema"]}.temp_DIM_CONTACT_FINAL_CONSOLIDATION_updated
""")
