...
table_name  = "DIM_CONTACT_FINAL_CONSOLIDATION"
...
/* ==============[Group.12 + 13 + 14: GUAVA_CUSTOMER, GUAVA_COMPANY, Merge Account Contact]============== */
    DROP TABLE IF EXISTS {params["cur_schema"]}.TEMP_DIM_ACCOUNT_CONTACT_GUAVA
""")
spark.sql(f"""
CREATE TABLE {params["cur_schema"]}.TEMP_DIM_ACCOUNT_CONTACT_GUAVA (
      OWNER_ID VARCHAR(50), /* None */
      CONTACT_OWNER_TYPE VARCHAR(20), /* None */
      CONTACT_TYPE VARCHAR(15), /* None */
      CONTACT_VALUE VARCHAR(150), /* None */
...
/* ==============[Group.12a - GUAVA CUSTOMER - CONTACT TYPE]============== */
    INSERT INTO {params["cur_schema"]}.TEMP_DIM_ACCOUNT_CONTACT_GUAVA (
      OWNER_ID, /* None */
      CONTACT_OWNER_TYPE, /* None */
      CONTACT_TYPE, /* None */
      CONTACT_VALUE, /* None */
...
/* ==============[Group.13a - GUAVA COMPANY - CONTACT TYPE]============== */
    ...
    INNER JOIN (
      SELECT
        ...
        EP.CONTACT_TYPE,
        ROW_NUMBER() OVER (
          PARTITION BY EP.BRANCH_NAME, EP.CONTACT_TYPE
          ORDER BY EP.EXTERNALCONTACT_PHONENUMBER DESC
        ) AS SEQUENCE_NO
      FROM (
        SELECT
          R.BRANCH_NAME,
        ...
        FROM {params["com_schema"]}.T_GUAVA_BRANCH_CONTACT AS R
        WHERE
          DATE_FORMAT(R.dl_record_updated_date, 'yyyyMMdd') = '{batch_date}'
...
/* ==============[Group.14: Merge GUAVA Account Contact]============== */
    WITH temp_dim_account_contact_guava_row_num AS (
      SELECT
        ...
        CONTACT_VALUE,
        CONTACT_NAME,
        DATE_FORMAT(CONTACT_CREATE_DATE, 'yyyy-MM-dd') AS CONTACT_CREATE_DATE, /* None */
        DATE_FORMAT(CONTACT_UPDATE_DATE, 'yyyy-MM-dd') AS CONTACT_UPDATE_DATE, /* None */
        LINE_OF_BUSINESS,
        SOURCE_NAME,
        SOURCE_RECORD_ID,
        SEQUENCE_NO,
        ROW_NUMBER() OVER (PARTITION BY OWNER_ID, CONTACT_OWNER_TYPE, CONTACT_TYPE ORDER BY PRIORITY) AS rn
      FROM {params["cur_schema"]}.TEMP_DIM_ACCOUNT_CONTACT_GUAVA
    )
    INSERT INTO {params["cur_schema"]}.TEMP_DIM_ACCOUNT_CONTACT (
      OWNER_ID, /* None */
      CONTACT_OWNER_TYPE, /* None */
...
/* ==============[Group.35]============== */
    ...
    SELECT
      ...
      T1.ACCOUNT_NO AS SOURCE_RECORD_ID, /* None */
      1 AS SEQUENCE_NO /* None */
    FROM {params["com_schema"]}.T_KDI_CUSTOMER AS T1 /* None */
    WHERE
      DATE_FORMAT(T1.dl_record_updated_date, 'yyyyMMdd') = '{batch_date}'
      AND NOT T1.clean_rule_flag LIKE '%1%'
      AND COALESCE(T1.CUSTOMER_CONTACT_NUMBER, '') <> ''
...
""")
spark.sql(f"""
INSERT INTO {params["cur_schema"]}.TEMP_DIM_CUSTOMER_CONTACT (
      OWNER_ID, /* None */
      CONTACT_OWNER_TYPE, /* None */
      CONTACT_TYPE, /* None */
      CONTACT_VALUE, /* None */
    ...
    FROM (
      SELECT
        ...
        ROW_NUMBER() OVER (
          ...
          ORDER BY COALESCE(DAC.CONTACT_UPDATE_DATE, DAC.CONTACT_CREATE_DATE) DESC
        ) AS ROWNUM
      ...
      INNER JOIN {params["cur_schema"]}.TEMP_DIM_ACCOUNT_CONTACT AS DAC
...
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_DIM_CONTACT_FINAL_CONSOLIDATION_delta""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_DIM_CONTACT_FINAL_CONSOLIDATION_delta (
        owner_id VARCHAR(50)
...
# ─── UPDATED TABLE SETUP ─────────────────────────────────────────────────────
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_DIM_CONTACT_FINAL_CONSOLIDATION_updated""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_DIM_CONTACT_FINAL_CONSOLIDATION_updated (
        owner_id VARCHAR(50)
        , contact_owner_type VARCHAR(20)
        , contact_type VARCHAR(10)
        , contact_value VARCHAR(150)
...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_DIM_CONTACT_FINAL_CONSOLIDATION_updated
    SELECT
        cur.owner_id,
    ...
    FROM {params["cur_schema"]}.DIM_CONTACT_FINAL_CONSOLIDATION cur
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_DIM_CONTACT_FINAL_CONSOLIDATION_delta
    ) impacted_partitions
    ON cur.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
cur.['source_key'] = 'FINAL_CONSOLIDATION' AND         NOT EXISTS (
            SELECT 1 FROM {params["com_schema"]}.temp_DIM_CONTACT_FINAL_CONSOLIDATION_delta delta
...
spark.sql(f"""
    ...
    FROM {params["com_schema"]}.temp_DIM_CONTACT_FINAL_CONSOLIDATION_delta delta
    LEFT JOIN {params["cur_schema"]}.DIM_CONTACT_FINAL_CONSOLIDATION cur
        ON cur.['source_key'] = 'FINAL_CONSOLIDATION' AND delta.OWNER_ID = cur.OWNER_ID AND delta.CONTACT_OWNER_TYPE = cur.CONTACT_OWNER_TYPE AND delta.CONTACT_TYPE = cur.CONTACT_TYPE""")

# ─── STEP 3: Insert previous state of changed records into History table ─────
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.DIM_CONTACT_FINAL_CONSOLIDATION_h PARTITION (['source_key'], <<partition_column>>, hist_year)
    SELECT
        cur.owner_id,
        cur.contact_owner_type,
        cur.contact_type,
        ...
        current_timestamp() AS etl_timestamp
, 'FINAL_CONSOLIDATION' AS ['source_key']        , cur.<<partition_column>>
    ...
    ON cur.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
cur.['source_key'] = 'FINAL_CONSOLIDATION' AND         EXISTS (
            SELECT 1 FROM {params["com_schema"]}.temp_DIM_CONTACT_FINAL_CONSOLIDATION_delta delta
            WHERE delta.OWNER_ID = cur.OWNER_ID AND delta.CONTACT_OWNER_TYPE = cur.CONTACT_OWNER_TYPE AND delta.CONTACT_TYPE = cur.CONTACT_TYPE        )
""")
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.DIM_CONTACT_FINAL_CONSOLIDATION PARTITION (['source_key'] = 'FINAL_CONSOLIDATION', <<partition_column>>)
    SELECT
        owner_id,
        contact_owner_type,
        contact_type,
        ...
        current_timestamp() AS etl_timestamp
        , <<partition_column>>
    FROM {params["com_schema"]}.temp_DIM_CONTACT_FINAL_CONSOLIDATION_updated
""")

spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.DIM_CONTACT_FINAL_CONSOLIDATION COMPUTE STATISTICS
""")
