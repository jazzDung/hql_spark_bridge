...
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.DIM_CONTACT_GUAVA_h PARTITION (['source_key'], hist_year)
    SELECT
        cur.owner_id,
        cur.contact_owner_type,
        cur.contact_type,
        ...
        current_timestamp() AS etl_timestamp
, 'GUAVA' AS ['source_key']        , DATE_FORMAT(current_timestamp(), 'yyyy') AS hist_year
    FROM {params["cur_schema"]}.DIM_CONTACT_GUAVA cur
    WHERE
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.DIM_CONTACT_GUAVA PARTITION (['source_key'] = 'GUAVA')    SELECT
        owner_id,
        contact_owner_type,
        contact_type,
        contact_value,
...
spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.DIM_CONTACT_GUAVA PARTITION (['source_key'] = 'GUAVA') COMPUTE STATISTICS
""")
