...
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_DIM_CONTACT_RAK_delta""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_DIM_CONTACT_RAK_delta (
        owner_id VARCHAR(50)
...
# ─── UPDATED TABLE SETUP ─────────────────────────────────────────────────────
spark.sql(f"""DROP TABLE IF EXISTS {params["com_schema"]}.temp_DIM_CONTACT_RAK_updated""")
spark.sql(f"""
    CREATE TABLE {params["com_schema"]}.temp_DIM_CONTACT_RAK_updated (
        owner_id VARCHAR(50)
        , contact_owner_type VARCHAR(20)
        , contact_type VARCHAR(10)
        , contact_value VARCHAR(150)
...
spark.sql(f"""
    INSERT INTO TABLE {params["com_schema"]}.temp_DIM_CONTACT_RAK_updated
    SELECT
        cur.owner_id,
    ...
    FROM {params["cur_schema"]}.DIM_CONTACT_RAK cur
    INNER JOIN (
        SELECT DISTINCT <<partition_column>>        FROM {params["com_schema"]}.temp_DIM_CONTACT_RAK_delta
    ) impacted_partitions
    ON cur.<<partition_column>> = impacted_partitions.<<partition_column>>    WHERE
cur.['source_key'] = 'RAK' AND         NOT EXISTS (
            SELECT 1 FROM {params["com_schema"]}.temp_DIM_CONTACT_RAK_delta delta
...
spark.sql(f"""
    ...
    FROM {params["com_schema"]}.temp_DIM_CONTACT_RAK_delta delta
    LEFT JOIN {params["cur_schema"]}.DIM_CONTACT_RAK cur
        ON cur.['source_key'] = 'RAK' AND delta.OWNER_ID = cur.OWNER_ID AND delta.CONTACT_OWNER_TYPE = cur.CONTACT_OWNER_TYPE AND delta.CONTACT_TYPE = cur.CONTACT_TYPE""")

# ─── STEP 3: Insert previous state of changed records into History table ─────
spark.sql(f"""
    INSERT INTO TABLE {params["cur_schema"]}.DIM_CONTACT_RAK_h PARTITION (['source_key'], <<partition_column>>, hist_year)
    SELECT
        cur.owner_id,
        cur.contact_owner_type,
        cur.contact_type,
        ...
        current_timestamp() AS etl_timestamp
, 'RAK' AS ['source_key']        , cur.<<partition_column>>
...
spark.sql(f"""
    INSERT OVERWRITE TABLE {params["cur_schema"]}.DIM_CONTACT_RAK PARTITION (['source_key'] = 'RAK', <<partition_column>>)
    SELECT
        owner_id,
        contact_owner_type,
        contact_type,
        ...
        current_timestamp() AS etl_timestamp
        , <<partition_column>>
    FROM {params["com_schema"]}.temp_DIM_CONTACT_RAK_updated
""")

spark.sql(f"""
    ANALYZE TABLE {params["cur_schema"]}.DIM_CONTACT_RAK COMPUTE STATISTICS
""")
